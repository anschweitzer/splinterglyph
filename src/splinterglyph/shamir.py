...
def split_secret(secret_bytes, required_shares, distributed_shares, **kwargs):
    if required_shares > distributed_shares:
        raise ValueError(
            "distributed_shares must be greater than or equal to required_shares"
        )
    ...
    prime_mod = kwargs.get(
        "prime_mod", util.select_prime_larger_than(largest_representable_secret)
    )
    ...
    with kwargs.get(
        "randomness_source",
        randomness.RandomReader()
        if secret_length <= 65
        else randomness.UrandomReader(),
    ) as randomness_source:
        secret = util.int_from_bytes(secret_bytes)
        coefficients = []
        for i in range(1, required_shares):
            coefficients.append(
                util.int_from_bytes(randomness_source.next_bytes(prime_bytes))
                % prime_mod
            )
...
def recover_secret(data):
    ...
    if required_shares:
        ...
        warnings.warn(
            "The number of required shares has not been specified. If not enough shares are provided, an incorrect secret will be produced without detection."
        )
    ...
    for x, y in shares:
        ...
        if not isinstance(y, bytes):
            raise TypeError(
                "the second entry of each a share must be an array of bytes"
            )
...
def from_base64(data):
    ...
    return {
        ...
        if isinstance(data["prime_mod"], int)
